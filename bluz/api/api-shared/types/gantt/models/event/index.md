[**TypeDoc API**](../../../../../index.md)

***

[TypeDoc API](../../../../../index.md) / api-shared/types/gantt/models/event

# api-shared/types/gantt/models/event

## Enumerations

- [EventRecurrence](enumerations/EventRecurrence.md)
- [ModuleEventType](enumerations/ModuleEventType.md)
- [RoomRequirement](enumerations/RoomRequirement.md)

## Type Aliases

- [GanttEvent](type-aliases/GanttEvent.md)
- [GanttEventId](type-aliases/GanttEventId.md)

## Functions

- [defaultModuleEventSplitAcrossBreaks](functions/defaultModuleEventSplitAcrossBreaks.md)
